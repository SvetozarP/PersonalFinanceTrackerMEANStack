...
export class AuthService {
    ...
    async login(credentials: ILoginCredentials): Promise<{ user: IUser; tokens: IAuthTokens }> {
        ...
        // Generate tokens
        const tokens = await this.generateTokens((user._id as mongoose.Types.ObjectId).toString());
    ...
    async refreshToken(refreshToken: string): Promise<IAuthTokens> {
        try {
            const decoded = jwt.verify(refreshToken, this.JWT_SECRET as jwt.Secret) as jwt.JwtPayload & { userId: string };
            ...
            return await this.generateTokens((user._id as mongoose.Types.ObjectId).toString());
        } catch (error) {
            throw new Error('Invalid refresh token');
        }
    }
    ...
    async validateToken(token: string): Promise<{ userId: string }> {
        try {
            const decoded = jwt.verify(token, this.JWT_SECRET as jwt.Secret) as jwt.JwtPayload & { userId: string; type: string };
