...
import { ITransaction, TransactionType, TransactionStatus, RecurrencePattern } from '../interfaces/transaction.interface';
...
export class TransactionService {
   ...
   */
  async createTransaction(transactionData: Partial<ITransaction>, userId: string): Promise<ITransaction> {
    try {
      ...
      const newTransaction = await this.transactionRepository.create({
        ...
        recurrencePattern: transactionData.recurrencePattern || RecurrencePattern.NONE,
      ...
      if (newTransaction.isRecurring && newTransaction.recurrencePattern !== RecurrencePattern.NONE) {
      ...
      logger.error('Error creating transaction', { 
        error: String(error), 
        userId, 
        transactionData: { title: transactionData.title, amount: transactionData.amount } 
      });
      throw error;
    }
  }
  ...
  async getUserTransactions(userId: string, options: {
    ...
    try {
      ...
      const { 
        type, status, categoryId, subcategoryId, paymentMethod, isRecurring, source,
   ...
   * Update transaction
   */
  async updateTransaction(transactionId: string, updateData: Partial<ITransaction>, userId: string): Promise<ITransaction> {
    try {
      ...
      if (updateData.categoryId && updateData.categoryId.toString() !== existingTransaction.categoryId.toString()) {
        // Validate category belongs to user (you'll need to implement category service integration)
        // const categoryService = new CategoryService();
      ...
      if (updateData.isRecurring !== undefined || updateData.recurrencePattern !== undefined) {
   ...
   */
  async bulkCreateTransactions(transactionsData: Partial<ITransaction>[], userId: string): Promise<ITransaction[]> {
    try {
      logger.info('Bulk creating transactions', { userId, count: transactionsData.length });

      const createdTransactions: ITransaction[] = [];

      for (const transactionData of transactionsData) {
        try {
          const transaction = await this.createTransaction(transactionData, userId);
  ...
  async getTransactionStats(userId: string, options: {
    ...
    transactionsByType: { [key in TransactionType]: { count: number; total: number } };
    ...
    try {
      ...
      if (categoryId) baseQuery.categoryId = new mongoose.Types.ObjectId(categoryId);
      if (type) baseQuery.type = type;

      // Get basic stats
      const [totalTransactions, totalIncome, totalExpenses, totalTransfers, totalAdjustments] = await Promise.all([
      ...
      const stats = {
        ...
        averageTransactionAmount: totalTransactions > 0 ? 
          (totalIncome[0]?.total || 0) / totalTransactions : 0,
   ...
   */
  private async createRecurringSeries(transaction: ITransaction): Promise<void> {
    try {
      if (!transaction.isRecurring || transaction.recurrencePattern === RecurrencePattern.NONE) {
        return;
      }

      const endDate = transaction.recurrenceEndDate;
      if (!endDate) {
        throw new Error('Recurrence end date is required for recurring transactions');
      }
   ...
   */
  private formatTransactionsByType(aggregationResult: any[]): { [key in TransactionType]: { count: number; total: number } } {
    const result: any = {
